...
const util = {
    css: (elem: HTMLDivElement, obj: any) => {
    ...
    //     return elem.getAttribute("class").indexOf(classN) != -1;
    // }
};

export class ColorPicker {

    private bindElem: HTMLElement = null; // 绑定的元素
    private elem_wrap: HTMLDivElement = null; // 最外层容器
    private fixedBg: HTMLDivElement = null; // 拾色器后面固定定位的透明div 用于点击隐藏拾色器
    private elem_colorPancel: HTMLDivElement = null; // 色彩面板
    private elem_picker: HTMLDivElement = null; // 拾色器色块按钮
    private elem_barPicker1: HTMLDivElement = null; // 颜色条
    private elem_barPicker2: HTMLDivElement = null; // 透明条
    private elem_hexInput:HTMLInputElement = null; // 显示hex的表单
    private elem_showColor: HTMLDivElement = null; // 显示当前颜色
    private elem_opacityPancel:HTMLDivElement = null; // 透明度背景元素
    private elem_showModeBtn:HTMLDivElement = null; // 切换输入框模式按钮
    private elem_inputWrap:HTMLDivElement = null; // 输入框外层容器

    private pancel_width: number;
    private pancel_height: number;
    ...
    private current_mode = 'hex'; // input框当前的模式
    ...
    init(): void {
        const _this = this;
        let initColor = this.Opt.initColor;
        let rgb = initColor.slice(4, -1).split(",");
        ...
        this.elem_wrap = div;
        this.fixedBg = <HTMLDivElement>(div.children[0]);
        this.elem_colorPancel = <HTMLDivElement>(div.getElementsByClassName("color-pancel")[0]);
        this.pancel_width = this.elem_colorPancel.offsetWidth;
        this.pancel_height = this.elem_colorPancel.offsetHeight;
        this.elem_picker = <HTMLDivElement>(div.getElementsByClassName("pickerBtn")[0]);
        this.elem_showColor = <HTMLDivElement>(div.getElementsByClassName("colorpicker-showColor")[0]);
        this.elem_barPicker1 = <HTMLDivElement>(div.getElementsByClassName("colorBar-color-picker")[0]);
        this.elem_barPicker2 = <HTMLDivElement>(div.getElementsByClassName("colorBar-opacity-picker")[0]);
        this.elem_hexInput = <HTMLInputElement>(div.getElementsByClassName("colorpicker-hexInput")[0]);
        this.elem_showModeBtn = <HTMLDivElement>(div.getElementsByClassName("colorpicker-showModeBtn")[0]);
        this.elem_inputWrap = <HTMLDivElement>(div.getElementsByClassName("colorpicker-inputWrap")[0]);
        this.elem_opacityPancel = <HTMLDivElement>(this.elem_barPicker2.parentNode.parentNode.children[1]);

        // var rect = this.bindElem.getBoundingClientRect();
        let elem = this.bindElem;
        let top = elem.offsetTop;
        let left = elem.offsetLeft;
        let elemOffsetParent:HTMLElement;
        while (elem.offsetParent) {
            elemOffsetParent = <HTMLElement>elem.offsetParent;
            top += elemOffsetParent.offsetTop;
            left += elemOffsetParent.offsetLeft;
        ...
        this.bindMove(<HTMLDivElement>(this.elem_barPicker1.parentNode), this.setBar, false);
        this.bindMove(<HTMLDivElement>(this.elem_barPicker2.parentNode), this.setBar, false);
    ...
    bindMove(elem: HTMLDivElement, fn: Function, bool: boolean) {
        const _this = this;
    ...
    show() {
        util.css(<HTMLDivElement>this.elem_wrap,{
            "display":"block"
        })
    }
